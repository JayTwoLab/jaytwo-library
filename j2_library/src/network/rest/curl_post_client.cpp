...
void curl_post_client::global_init()
{
...
size_t curl_post_client::write_callback(char* ptr, size_t size, size_t nmemb, void* userdata)
{
...
size_t curl_post_client::header_callback(char* buffer, size_t size, size_t nitems, void* userdata)
{
...
curl_post_client::curl_post_client()
{
    global_init();
...
void curl_post_client::set_server(const std::string& scheme,
...
void curl_post_client::set_headers(const headers& headers)
{
    headers_ = headers;
}

void curl_post_client::set_timeout_ms(long timeout_ms)
{
    if (timeout_ms <= 0) throw std::invalid_argument("timeoutMs must be greater than 0.");
    timeout_ms_ = timeout_ms;
}

void curl_post_client::set_ignore_ssl_errors(bool ignore)
{
    ignore_ssl_errors_ = ignore;
}

std::vector<std::string> curl_post_client::build_header_lines(const headers& headers)
{
...
curl_post_client::http_status curl_post_client::to_http_status(long code)
{
    switch (code) {
...
std::string curl_post_client::build_url(const query_params& query_params)
{
    if (host_.empty())
...
//  - Send the body string as is
//  - Content-Type and other headers are set via set_headers()
// ------------------------------
curl_post_client::response curl_post_client::post(const std::string& body_str)
{
    if (!curl_)
    ...
    // For POST, request is made based only on path without query string (can be extended if needed)
    ...
    // Set the body to be sent (JSON, XML, TEXT, etc.)
...
curl_post_client::result_code curl_post_client::classify(const response& r)
{
    if (!r.error.empty()) {
...
// ------------------------------
curl_post_client::result_code
    curl_post_client::post(const std::string& body_str, response& out_resp) noexcept
{
    try {
