...
TEST(Crypt, EncryptDecrypt_Basic) {
    ...
    const std::string original = "Standard AES-128 Implementation Test!"; // 테스트할 원본 문자열
    const std::string encrypted = cipher.encrypt(original); // 암호화된 결과는 헥스 문자열로 반환됨
    ...
    EXPECT_EQ(original, decrypted); // 복호화된 결과가 원본과 일치해야 함
}
