...
protected:
    void SetUp() override { // 각 테스트 시작 전에 자동 호출 (공통 초기화)
        ...
        // 2) 다양한 케이스 재현을 위해 파일에 추가
        {
            ...
            app << "truthy_raw=true\n";        // bool true 로 인식 (parser는 true/false만 허용)
            app << "falsey_raw=false\n";       // bool false 로 인식
    ...
    j2::ini::ini_parser cfg2_; // 재로드된 설정 객체. 각 테스트에서 사용.
};
