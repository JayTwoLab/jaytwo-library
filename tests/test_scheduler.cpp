...
TEST(SchedulerCases, Tue23_To_Wed10_Wed09_ActiveAndJsonXml) {
    ...
    j2::schedule::weekly::scheduler s;

    // 화요일 23:00 부터 수요일 10:00 까지
    // 매주마다 반복되는 스케쥴 범위 설정
    j2::schedule::weekly::weekly_range r{
        j2::schedule::weekly::weekday::tue,   j2::schedule::weekly::time_hm{23, 0},
        j2::schedule::weekly::weekday::wed,   j2::schedule::weekly::time_hm{10, 0}
    };
    s.add_range(r); // 스케쥴 범위 추가

    // 변경: time_point 프로바이더 사용 (로컬 기준으로 tm -> time_point 변환)
    auto provider_tp = []() {
        std::tm tm = j2::schedule::weekly::make_tm_with_wday_hour_min(
            j2::schedule::weekly::weekday::wed, 9, 0);
        return j2::datetime::to_timepoint(tm, j2::datetime::time_zone_mode::local_time);
    };
    s.set_now_provider(provider_tp); // 현재 시각 제공자 설정
    EXPECT_TRUE(s.is_active_now()); // 현재 시간이 스케쥴 범위 내인지 확인 (수요일 09:00 이므로 범위 내부임)   

    // JSON 출력 검사
    auto j = j2::schedule::weekly::to_json(s.ranges()); // 현재 설정된 스케쥴 범위들을 JSON으로 변환
    ...
    auto xml = j2::schedule::weekly::to_xml(s.ranges()); // 현재 설정된 스케쥴 범위들을 XML(문자열)로 변환
...
TEST(SchedulerCases, Sat23_To_Mon08_Mon07_ActiveAndJsonXml) {
    ...
    j2::schedule::weekly::weekly_range r{
        j2::schedule::weekly::weekday::sat,   j2::schedule::weekly::time_hm{23, 0},
        j2::schedule::weekly::weekday::mon,   j2::schedule::weekly::time_hm{8, 0}
    };
    s.add_range(r); // 스케쥴 범위 추가
    ...
    auto provider_tp = []() {
        std::tm tm = j2::schedule::weekly::make_tm_with_wday_hour_min(
            j2::schedule::weekly::weekday::mon, 7, 0);
        return j2::datetime::to_timepoint(tm, j2::datetime::time_zone_mode::local_time);
    };
    s.set_now_provider(provider_tp); // 현재 시각 제공자 설정
    ...
    auto j = j2::schedule::weekly::to_json(s.ranges()); // 스케쥴 범위들을 JSON으로 변환
    ...
    auto xml = j2::schedule::weekly::to_xml(s.ranges()); // 스케쥴 범위들을 XML(문자열)로 변환
    std::cout << xml << std::endl;
    EXPECT_NE(xml.find("WeeklySchedule"), std::string::npos);
...
TEST(SchedulerCases, MergeOverlappingRanges_Mon10_13_and_Mon11_12_MergedDump) {
    ...
    // 월요일 10:00~13:00     
    j2::schedule::weekly::weekly_range a{
        j2::schedule::weekly::weekday::mon,   j2::schedule::weekly::time_hm{10, 0},
        j2::schedule::weekly::weekday::mon,   j2::schedule::weekly::time_hm{13, 0}
    };

    // 월요일 11:00~12:00
    j2::schedule::weekly::weekly_range b{
        j2::schedule::weekly::weekday::mon,   j2::schedule::weekly::time_hm{11, 0},
        j2::schedule::weekly::weekday::mon,   j2::schedule::weekly::time_hm{12, 0}
    };

    j2::schedule::weekly::weekly_ranges in{a, b};
    auto normalized = j2::schedule::weekly::schedule_normalizer::normalize(in);

    // 병합되면 하나의 항목으로 합쳐져야 함
    ASSERT_EQ(normalized.size(), 1u); // 겹치지 않으면 2

    auto dump = j2::schedule::weekly::dump_schedule(normalized);
...
TEST(SchedulerCases, SetNow_UpdatesTimeAndIsActive) {
    ...
    j2::schedule::weekly::weekly_range r{
        j2::schedule::weekly::weekday::mon,   j2::schedule::weekly::time_hm{8, 0},
        j2::schedule::weekly::weekday::mon,   j2::schedule::weekly::time_hm{9, 0}
    };
    s.add_range(r); // 스케쥴 범위 추가

    // 시각을 월요일 07:30로 설정하면 비활성
    s.set_now(j2::schedule::weekly::weekday::mon, 7, 30);
    EXPECT_FALSE(s.is_active_now());

    // 같은 인스턴스에서 시각을 월요일 08:30로 변경하면 활성
    s.set_now(j2::schedule::weekly::weekday::mon, 8, 30);
    EXPECT_TRUE(s.is_active_now());

    // 다시 화요일로 변경하면 비활성
    s.set_now(j2::schedule::weekly::weekday::tue, 8, 30);
    EXPECT_FALSE(s.is_active_now());
...
TEST(SchedulerCases, AddRange_MergedThenSetNowSequence) {
    ...
    j2::schedule::weekly::scheduler s(j2::schedule::weekly::time_base::localtime);
    ...
    s.set_now(j2::schedule::weekly::weekday::mon, 9, 0);
    EXPECT_FALSE(s.is_active_now());

    // then set_now to a time inside merged interval -> should be active
    s.set_now(j2::schedule::weekly::weekday::mon, 11, 30);
    EXPECT_TRUE(s.is_active_now());
}
...
TEST(SchedulerCases, LoadFromJson_PopulatesAndMerges) {
    ...
    s.set_now(j2::schedule::weekly::weekday::mon, 9, 30);
    EXPECT_FALSE(s.is_active_now());

    s.set_now(j2::schedule::weekly::weekday::mon, 11, 0);
    EXPECT_TRUE(s.is_active_now());
}
...
TEST(SchedulerCases, SetNowWithTimePoint_Works) {
    ...
    j2::schedule::weekly::scheduler s(j2::schedule::weekly::time_base::localtime); // Localtime 기준 스케쥴러 생성
    ...
    auto tp_inside = j2::datetime::to_timepoint(
        j2::schedule::weekly::make_tm_with_wday_hour_min(
            j2::schedule::weekly::weekday::mon, 11, 0), 
    ...
    auto tp_outside = j2::datetime::to_timepoint(
        j2::schedule::weekly::make_tm_with_wday_hour_min(
            j2::schedule::weekly::weekday::mon, 9, 0), 
        j2::datetime::time_zone_mode::local_time
    );
    ...
    // 실제 현재 시각으로 설정하는 테스트
    j2::schedule::weekly::scheduler s_now(j2::schedule::weekly::time_base::localtime);  
